...
class NetEase(object):
    ...
    @property
    def toplists(self):
        return [l[0] for l in c.TOP_LIST_ALL.values()]
    ...
    def _raw_request(self, method, endpoint, data=None):
        if method == 'GET':
            resp = self.session.get(endpoint, params=data, headers=self.header, timeout=c.DEFAULT_TIMEOUT)
        elif method == 'POST':
            resp = self.session.post(endpoint, data=data, headers=self.header, timeout=c.DEFAULT_TIMEOUT)
        return resp
